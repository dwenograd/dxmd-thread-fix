// path_util.cpp - long-path-safe module path helpers.
//
// Uses the MSDN-recommended double-the-buffer-and-retry pattern for
// GetModuleFileNameW, capped at 32K wchars.
...
wchar_t* get_module_path(HMODULE module) {
    ...
    // the buffer size means the result was truncated (it doesn't report
    // the required size). Grow and retry. A fixed MAX_PATH buffer would
    // silently fail on long-path-enabled installs and break host
    // detection, config discovery, and logging.
    ...
    for (;;) {
        ...
        // Truncated; grow and retry, capped at MAX_BUF.
    ...
    // Never return a truncated buffer.
...
wchar_t* get_module_dir(HMODULE module) {
    ...
    if (!path) return nullptr;
    // Strip filename, leave trailing separator.
    size_t len = 0;
    while (path[len]) ++len;
